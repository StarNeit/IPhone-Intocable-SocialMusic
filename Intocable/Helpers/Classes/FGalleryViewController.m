...
- (void)loadView
{
    ...
    _caption.shadowColor						= [UIColor whiteColor];
...
{
    [[UINavigationBar appearance] setBarTintColor:[UIColor colorWithRed:27/255.0f green:57/255.0f blue:187.0f alpha:1.0]];
    UIButton *button1=[UIButton buttonWithType:UIButtonTypeCustom];
    [button1 setFrame:CGRectMake(0, 2.0, 13.0, 21.0)];
    [button1 addTarget:self action:@selector(doback) forControlEvents:UIControlEventTouchUpInside];
    [button1 setImage:[UIImage imageNamed:@"ic_btn_back.png"] forState:UIControlStateNormal];
...
- (void)updateTitle
{
    if (!_hideTitle){
        ...
        [[UINavigationBar appearance] setTitleTextAttributes: [NSDictionary dictionaryWithObjectsAndKeys:
                                                               ...
                                                               [UIFont fontWithName:@"HelveticaNeue-Light" size:17.0], NSFontAttributeName, nil]];
    }else{
        [self setTitle:@""];
    }
}
...
- (void)buildThumbsViewPhotos
{
	...
	for (i = 0; i < count; i++) {
		...
		[thumbView setTag:i];
         thumbView.center =self.view.center;
...
- (void)galleryPhoto:(FGalleryPhoto*)photo didLoadThumbnail:(UIImage*)image
{
    ...
    photoView.frame = CGRectMake(0,0, photo.thumbnail.size.width,  photo.thumbnail.size.height);
